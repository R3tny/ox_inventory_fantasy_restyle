		...
		description = %q
				...
				for _, item in pairs(dump) do
					if not ItemList[item.name] then
						...
						local itemStr = itemFormat:format(item.name, item.label, item.tipologia, item.rarity, item
						.weight, item.stack, item.close, item.description and json.encode(item.description) or 'nil')
